...
To develop bitcore-node-pac:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-pac.git
...
We now will setup symlinks in `bitcore-node-pac` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
...
cd devnode
mkdir node_modules
touch bitcore-node-pac.json
touch package.json
```

Edit `bitcore-node-pac.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-pac/bin/bitcore-node-pac start
```