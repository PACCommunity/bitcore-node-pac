...
Bitcoin.prototype.getInfo = function(callback) {
  ...
  var getNetworkHashps = function(next) {
    self.client.getNetworkHashps(function(err, response) {
      if(response && response.hasOwnProperty('result')) {
        var hashrate = parseFloat(response.result);
        rawResult.networkHashRate = Math.round(hashrate * 10) / 10;
      }
      next();
    });
  };
  ...
  var prepareResponse = function(err){
    ...
    if (keys.indexOf('version') > -1 && keys.indexOf('networkHashRate')> -1){
      return callback(null, rawResult);
    } else {
      return callback(new Error('Invalid GetInfo'), null);
    }
  };
  return async.series([getInfo, getNetworkHashps], prepareResponse);
};
